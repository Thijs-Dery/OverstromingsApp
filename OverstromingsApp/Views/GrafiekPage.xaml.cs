...
public partial class GrafiekPage : ContentPage
{
    ...
    public GrafiekPage(NeerslagController ctrl)
    {
        ...
        pinch.PinchUpdated += (s, e) => { if (e.Status == GestureStatus.Running) Zoom(e.Scale); };
    ...
    {
        _draw.ZoomFactor = Math.Clamp(_draw.ZoomFactor * (float)scale, 0.5f, 10f);
        ChartView.Invalidate();

        if (!_showingMonths && _draw.ZoomFactor >= ZoomMonthThreshold)
            ShowMonths();
        else if (_showingMonths && _draw.ZoomFactor < ZoomMonthThreshold)
            ShowYears();
    }
    ...
    {
        if (e.StatusType == GestureStatus.Started) _lastPanX = e.TotalX;
        else if (e.StatusType == GestureStatus.Running)
        {
            var dx = e.TotalX - _lastPanX; _lastPanX = e.TotalX;
